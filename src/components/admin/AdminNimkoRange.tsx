...
import { NimkoRangeSectionConfig } from '../../types';

export const AdminNimkoRange: React.FC = () => {
  const { nimkoRangeConfig, updateNimkoRangeConfig, products } = useStore();
  const [formData, setFormData] = useState<NimkoRangeSectionConfig>(nimkoRangeConfig);
  const [savedSuccess, setSavedSuccess] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    updateNimkoRangeConfig(formData);
  ...
  const nimkoProducts = products.filter(p => p.category === 'nimko');

  const handleSelectAllNimko = () => {
    const allNimkoIds = nimkoProducts.map(p => p.id);
    setFormData({ ...formData, customProductIds: allNimkoIds });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-[#E8DFC8] pb-4">
        <h2 className="text-xl font-bold text-[#2D1A16] font-display flex items-center gap-2">
          ...
          "Karachi's Favorite Mix Nimko Range" Section (CRUD)
        </h2>
        <p className="text-xs text-[#735A50]">
          Manage the artisanal tea-time Nimko spotlight on the homepage, customize section headings, badges, and choose specific nimko flavors.
        </p>
      </div>
      ...
      {savedSuccess && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 animate-in fade-in">
          ...
          <span>Nimko Range section settings saved successfully!</span>
        </div>
      )}

      <form onSubmit={handleSubmit} className="space-y-6">
        
        {/* Toggle Section Active & Texts */}
        <div className="bg-white p-6 rounded-2xl border border-[#E8DFC8] shadow-xs space-y-4">
          <div className="flex items-center justify-between p-4 bg-[#FAF7F0] rounded-xl border border-[#E8DFC8]">
            <div>
              <div className="text-sm font-bold text-[#2D1A16]">Enable Nimko Spotlight on Homepage</div>
              <div className="text-xs text-[#735A50]">Toggle visibility of the "Karachi's Favorite Mix Nimko Range" section</div>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Artisanal Tea-Time Crunch"
            ...
            <div>
              <label className="block text-xs font-bold text-[#3D2821] mb-1">Link CTA Text</label>
              <input
                type="text"
                value={formData.ctaText}
                onChange={(e) => setFormData({ ...formData, ctaText: e.target.value })}
                placeholder="e.g. View All Nimko Flavors"
                className="w-full bg-[#FAF7F0] border border-[#DDD4CA] focus:border-[#801414] text-xs font-semibold px-3 py-2.5 rounded-xl outline-none"
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="e.g. Karachi's Favorite Mix Nimko Range"
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                placeholder="e.g. Hand-prepared in small batches with pure spices, crunchy peanuts, and zero oily aftertaste."
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="text-sm font-bold text-[#2D1A16] font-display uppercase tracking-wider">
                Select Featured Nimko Products ({formData.customProductIds.length > 0 ? formData.customProductIds.length : 'All Nimko Category'})
              </h3>
              <p className="text-xs text-[#735A50]">
                Choose which crunchy snacks to spotlight. By default, all items in the 'nimko' category are displayed.
              </p>
            </div>
        ...
        <div className="flex justify-end pt-4 border-t border-[#E8DFC8]">
          ...
          >
            ...
            <span>Save Nimko Range Settings</span>
          </button>
        </div>
