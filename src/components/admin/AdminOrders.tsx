...
export const AdminOrders: React.FC = () => {
  const { orders, updateOrder, updateOrderStatus, deleteOrder, siteSettings } = useStore();
  ...
  const handleSendWhatsAppUpdate = (order: OrderDetails) => {
    const cleanPhone = order.phone.replace(/[^0-9]/g, '');
    const phoneWithCountry = cleanPhone.startsWith('92') ? cleanPhone : `92${cleanPhone.replace(/^0/, '')}`;

    const riderInfo = order.assignedRider?.name
      ? `\n🚴 Assigned Rider: *${order.assignedRider.name}* (${order.assignedRider.phone || ''})`
      : '';

    const msg = `Salam ${order.customerName}! 👋\n\nUpdate regarding your order *#${order.orderId}* from *${siteSettings.brandName}*:\n\n📌 Current Status: *${order.status}*\n💰 Total Bill: *Rs. ${order.total}* (${(order.paymentStatus || 'Paid').toUpperCase()})\n📍 Delivery Address: *${order.address}, ${order.area}*${riderInfo}\n\nThank you for choosing artisanal frozen kababs & fresh nimko! For inquiries, reply directly to this message.`;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-stone-500">
              Live tracking, rider assignment, line item cost & profit calculations, and WhatsApp notifications.
            </p>
          </div>
        </div>

        <div className="text-xs font-bold text-red-900 bg-red-50 px-3.5 py-1.5 rounded-full border border-red-200 font-mono">
              ...
              <tbody className="divide-y divide-stone-100">
                {filteredOrders.map(ord => (
                  <tr key={ord.orderId} className="hover:bg-stone-50 transition-colors">
                    ...
                    {/* Customer */}
                    <td className="py-3.5 px-4">
                      <div className="font-bold text-xs text-stone-900">{ord.customerName}</div>
                      <div className="text-[11px] text-stone-600 font-medium">{ord.phone}</div>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1.5 text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-200 transition-colors"
                        ...
                        <button
                          ...
                          className="p-1.5 text-red-700 bg-red-50 hover:bg-red-100 rounded-lg border border-red-200 transition-colors"
                          title="Delete record"
                        >
      ...
      {isEditingOrder && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-stone-200 space-y-5 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-stone-200 pb-3">
              <div>
                <h3 className="font-bold text-base text-stone-900">Edit Order #{isEditingOrder.orderId}</h3>
                <p className="text-xs text-stone-500">Update customer information, rider, and payment status</p>
              </div>
            ...
            <form onSubmit={handleSaveOrderEdit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-stone-700 mb-1">Fulfillment Status</label>
                  <select
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-stone-200">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-red-700 hover:bg-red-800 text-white rounded-xl font-bold shadow-xs"
                >
                  Save Changes
                </button>
              </div>
